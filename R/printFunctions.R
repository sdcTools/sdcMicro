...
definition = function(x, type = "freq", ...) {
  ...
  if (type == "freq") {
    ...
    cat("\n -  2-anonymity:  ")
    if (TFchange)
      cat(paste(sum(obj@risk$individual[, 2] < 2), "(orig: ", sum(obj@originalRisk$individual[,
        2] < 2), ")\n")) else cat(paste(sum(obj@risk$individual[, 2] < 2), "\n"))
    cat(" -  3-anonymity:  ")
    if (TFchange)
      cat(paste(sum(obj@risk$individual[, 2] < 3), "(orig: ", sum(obj@originalRisk$individual[,
        2] < 3), ")")) else cat(paste(sum(obj@risk$individual[, 2] < 3), ""))
    cat("\n--------------------------\n")
    n <- nrow(obj@origData)
    cat("\nPercentage of observations violating\n")
    cat(" -  2-anonymity:  ")
    if (TFchange)
      cat(paste(round(sum(obj@risk$individual[, 2] < 2)/n * 100, 2), "% ", "(orig: ",
        round(sum(obj@originalRisk$individual[, 2] < 2)/n * 100, 2), "%", ")\n")) else cat(paste(round(sum(obj@risk$individual[, 2] < 2)/n * 100, 2), "% \n"))
    cat(" -  3-anonymity:  ")
    if (TFchange)
      cat(paste(round(sum(obj@risk$individual[, 2] < 3)/n * 100, 2), "% ", "(orig: ",
        round(sum(obj@originalRisk$individual[, 2] < 3)/n * 100, 2), "%", ")")) else cat(paste(round(sum(obj@risk$individual[, 2] < 3)/n * 100, 2), "% \n"))
    ...
    cat("\n")
    cat("--------------------------\n")
    s <- sum((risk$individual[, 1] > median(risk$individual[, 1]) + 2 * mad(risk$individual[,
      1])) & (risk$indiviual[, 1] > 0.1))
    sorig <- sum((originalRisk$individual[, 1] > median(originalRisk$individual[, 1]) +
      2 * mad(originalRisk$individual[, 1])) & (originalRisk$indiviual[, 1] > 0.1))
    if (TFchange)
      cat(paste(s, " (orig:", sorig, ")", "obs. with higher risk than the main part\n")) else cat(paste(s, "obs. with higher risk than the main part\n"))
    cat("Expected no. of re-identifications:\n", round(risk$global$risk_ER, 2), "")
    if (TFchange)
      cat("[", round(risk$global$risk_pct, 2), "%]  (orig:", round(originalRisk$global$risk_ER,
        2), "[", round(originalRisk$global$risk_pct, 2), "%])\n") else cat("[", round(risk$global$risk_pct, 2), "%]\n")
    cat("--------------------------\n")
    if ("hier_risk_ER" %in% names(risk$global)) {
      if (!is.na(risk$global$hier_risk_ER)) {
        ...
        cat("--------------------------\n")
        cat("Expected no. of re-identifications:\n", round(risk$global$hier_risk_ER,
          2), "")
        if (TFchange)
          cat("[", round(risk$global$hier_risk_pct, 2), "%]  (orig:", round(originalRisk$global$hier_risk_ER,
          2), "[", round(originalRisk$global$hier_risk_pct, 2), "%])\n") else cat("[", round(risk$global$hier_risk_pct, 2), "%]  \n")
      } else {
        cat("--------------------------\n")
  ...
  } else stop(paste("type=", type, "is unknown."))
})
