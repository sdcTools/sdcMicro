...
#' @param weightVarHH name of the variable with the sampling weights in the anonymized household level dataset (dathh)
...
#' x_comb <- mergeHouseholdData(x, "ori_hid", x_hh, weightVarHH = "sampling_weight")
#' 
mergeHouseholdData <- function(dat, hhId, dathh, weightVarHH = NULL) {
  ...
  if (!is.null(weightVarHH))
    toBeDropped <- toBeDropped[-which(toBeDropped %in% weightVarHH)]
