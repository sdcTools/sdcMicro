localSupp2Wrapper <- function(x, keyVars, w, importance = rep(1, length(keyVars)), method = "minimizeSupp", 
  kAnon = 2) {
  ...
  while (indikator == FALSE) {
    tmp <- localSupp2(tmp, keyVars, w, importance = importance, method = "minimizeSupp", 
      k = kAnon)$xAnon
    supps.new <- sum(is.na(tmp))  #as.vector(apply(tmp, 2, function(x) { length(which(is.na(x)))}))
    ff <- freqCalc(tmp, keyVars, w)
    if ((ff$n1 == 0 & ff$n2 == 0) || supps.old == supps.new) {
      # (paste(supps.old, collapse='') == paste(supps.new, collapse=''))) {
      indikator <- TRUE
      supps <- sum(is.na(tmp))  #apply(tmp, 2, function(x) { length(which(is.na(x)))})
      # totalSupps <- sum(supps) rk <- indivRisk(ff)
    }
    supps.old <- supps.new
    i <- i + 1
  }
  ...
  x[is.na(tmp)] <- NA
  res <- list(xAnon = x, supps = supps, totalSupps = length(which(is.na(tmp))) - totalNA, 
    it = i, anonymity = anon, keyVars = keyVars, importance = importance, k = kAnon)
