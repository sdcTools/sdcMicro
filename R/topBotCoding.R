...
#'
setGeneric("topBotCoding", function(obj, value, replacement, kind = "top", column = NULL) {
  standardGeneric("topBotCoding")
})
...
definition = function(obj, value, replacement, kind = "top", column = NULL) {
  ...
  if (column %in% colnames(manipNumVars)) {
    ...
    manipNumVars[, column] <- topBotCodingWORK(x, value = value, replacement = replacement,
    ...
    manipKeyVars[, column] <- topBotCodingWORK(x, value = value, replacement = replacement,
      kind = kind, column = column)
    ...
    manipPramVars[, column] <- topBotCodingWORK(x, value = value, replacement = replacement,
      kind = kind, column = column)
  ...
  } else stop("variable could not be found in the data set")
  invisible(obj)
})
...
topBotCodingWORK <- function(x, value, replacement, kind = "top", column = NULL) {
  # COLUMN FOR COMPATIBILITY WITH V4
