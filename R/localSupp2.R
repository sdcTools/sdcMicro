localSupp2 <- function(x, keyVars, w, importance = rep(1, length(keyVars)), method = "minimizeSupp", 
  k = 1) {
  ## x ... matrix, no factors keyVar ... key variables w ... sampling weights method ...
  ## 'minimizeSupp', 'minimizeRisk' k ... k-anonymity importance ... weights for key variables
  ...
  # if( length(keyVarsNotSupp) > 0 ){ realVars <- keyVar[-which(keyVar %in% keyVarsNotSupp)]
  # }
  ...
  print(paste("estimated time for method fastSupp:", round(maxiter * system.time(zeit2(x))[1]), 
    "(in seconds)"))
    ...
    for (m in 1:k) {
      ...
      for (i in which(ff$fk == m)) {
        ...
        for (j in realVars) {
          actualObs <- x[i, j]
          x[i, j] <- NA
          ffs[j] <- length(which(freqCalc(x, keyVars = keyVars, w = w)$fk == m)) * 
          importanceI[j]
          x[i, j] <- actualObs
          ffs[which(is.na(x[i, ]))] <- NA
        }
  ...
  if (all(ff$fk >= k)) {
    ...
    cat("\n")
    print(paste(k, "-anonymity is not achieved. Re-apply localSupp2 on this result or use function localSupp2Wrapper ", 
      sep = ""))
    cat("\n")
    anon <- FALSE
  }
  ...
  res <- list(xAnon = y, supps = supps, totalSupps = length(which(is.na(x))) - totalNA, anonymity = anon, 
