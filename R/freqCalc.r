...
freqCalc <- function(x, keyVars, w=NULL, alpha=1) {
  ...
  . <- Fk <- fk <- id <- keyid <- na_ids <- sortidforfreqcalc <- NULL
  sortvar <- weight <- xxxxx_tmpweight_xxxxx <- NULL
  if (alpha < 0 | alpha > 1) {
  ...
  # check if all keyVars are factors; otherwise errors may occur
  # when later converting to numeric!
  for (v in keyVars_n) {
    if (!is.factor(dt[[v]])) {
      dt[[v]] <- as.factor(dt[[v]])
    }
  ...
  data.table::setkeyv(dt, keyVars_n)

  agg <- dt[, .(fk = .N, Fk = sum(weight)), by = key(dt)]
  TFna <- dt[, any(sapply(.SD, function(x) any(is.na(x)))), .SDcols = keyVars_n]
  if (!TFna) {
    data.table::setkeyv(agg, keyVars_n)
    data.table::setkeyv(dt, keyVars_n)
    agg <- agg[dt]
    data.table::setkey(agg, sortidforfreqcalc)
    ...
    z <- list(
      ...
      fk = agg$fk,
      Fk = agg$Fk,
      n1 = sum(agg$fk == 1, na.rm = TRUE),
      n2 = sum(agg$fk == 2, na.rm = TRUE),
      alpha = alpha
    )
    class(z) <- "freqCalc"
    return(z)
  ...
  if (length(keyVars_n) == 1) {
    ...
    new <- data.table::setkeyv(new, keyVars_n)
    new <- new[dt]
    data.table::setkey(new, sortidforfreqcalc)
    z <- list(
      freqCalc = x,
      keyVars = keyVars,
      w = w,
      fk = new$fk,
      Fk = new$Fk,
      n1 = sum(new$fk == 1, na.rm = TRUE),
      n2 = sum(new$fk == 2, na.rm = TRUE),
      alpha = alpha
    )
    class(z) <- "freqCalc"
    return(z)
  ...
  # initialize fk|Fk and add_fks|add_Fks for na and non-na
  # datasets with original values or 0
  ...
  for (i in 1:nrow(dat_with_na)) {
    ...
    if (i == 1 || (naind[i, keyid] != naind[i - 1, keyid])) {
      ...
      data.table::setkeyv(dat_without_na, cur_sortVars)
    }
  ...
  # in case we have NA-only keys, we add their (weighted) fks/Fks to all
  # keys with no missing values
  if (addAllNAtoKeys) {
  ...
  data.table::setkeyv(dat_without_na, keyVars_n)
