...
setGeneric('calcReportData', function(obj,
        pram.show,kAnon.show,indivRisk.show,hierRisk.show,riskNumKeyVars.show,dataUtility.show,
        localSupps.show,dataUtilityCont.show,sessionInfo.show,title) {
   standardGeneric('calcReportData')})

setMethod(f='calcReportData', signature=c('sdcMicroObj'), definition=function(obj,
        pram.show,kAnon.show,indivRisk.show,hierRisk.show,riskNumKeyVars.show,dataUtility.show,
        localSupps.show,dataUtilityCont.show,sessionInfo.show,title
        ) { 
    repObj <- new("reportObj")
    repObj@pram.show <- pram.show
    repObj@kAnon.show <- kAnon.show
    repObj@indivRisk.show <- indivRisk.show
    repObj@hierRisk.show <- hierRisk.show
    repObj@riskNumKeyVars.show <- riskNumKeyVars.show
    repObj@dataUtility.show <- dataUtility.show
    repObj@localSupps.show <- localSupps.show
    repObj@dataUtilityCont.show <- dataUtilityCont.show
    repObj@sessionInfo.show <- sessionInfo.show
    repObj@title <- title
    
    
### Only explicit parameters used -> ... - PART auskommentiert
    # parse args (...) from gui
#    args <- list(...)
#    
#    # set report title
#    cmd <- paste("repObj <- sdcMicro:::set.reportObj(repObj, 'title', list('",args[["title"]],"'))", sep="")
#    eval(parse(text=cmd))   
#    
#    poss.args <- setdiff(names(args), c("title", "format", "outdir","filename"))
#    args <- args[which(names(args) %in% poss.args)]
#    names(args) <- paste(names(args),".show", sep="")
#    
#    for ( i in 1:length(args) ) {
#       cmd <- paste("repObj <- sdcMicro:::set.reportObj(repObj, '",names(args)[i],"', list(",args[[i]],"))", sep="")
#       eval(parse(text=cmd))   
#    }   
    ...
    #######################################
    ### Disclosure Risk and k-Anonymity ###
    #######################################
...
setGeneric('report', function(obj, outdir=getwd(),filename="SDC-Report",format="HTML",title='SDC-Report',
        pram=TRUE,
        kAnon=TRUE,
        indivRisk=TRUE,
        hierRisk=TRUE,
...
setMethod(f='report', signature=c('sdcMicroObj'), 
    definition=function(obj, outdir=getwd(),filename="SDC-Report",format="HTML",title='SDC-Report',
        pram=TRUE,
        kAnon=TRUE,
        indivRisk=TRUE,
        hierRisk=TRUE,
       ...
       repObj <- calcReportData(obj, 
           pram.show=pram,
           kAnon.show=kAnon,
           indivRisk.show=indivRisk,
           hierRisk.show=hierRisk,
           riskNumKeyVars.show=riskNumKeyVars,
           dataUtility.show=dataUtility,
           localSupps.show=localSupps,
           dataUtilityCont.show=dataUtilityCont,
           sessionInfo.show=sessionInfo,title=title)
       ...
       if ( format == "HTML" ) {
          ...
          xx <- file.rename("reportTemplate-out.html", filename)
          xx <- file.remove("reportTemplate-out.md")
          setwd(oldwd)
       }
