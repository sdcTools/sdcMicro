...
output$ui_microaggregation <- renderUI({
  ...
  output$ui_microagg_clusterbased <- renderUI({
    ...
    txt_cltransf <- paste(txt_clmethod, "<strong>log</strong> - logarithmic transformation, which is centered around the mean and scaled by the standard deviation of each variable<br />")
    txt_cltransf <- paste(txt_clmethod, "<strong>boxcox</strong> - Box-Cox power transformation using a maximum likelihood approach<br />")
  ...
  output$ui_microagg_method <- renderUI({
    ...
    txt <- paste(txt, "<strong>Cluster-based methods:</strong> When using these methods, first clusters are constructed before applying the microaggregation method. Several clustering methods can be chosen as well as parameters for these methods.  <br />")
    txt <- paste(txt, "<strong>influence</strong> - grouping is based on the most influential variable within each cluster<br /><br />")
    txt <- paste(txt, "<strong>clustpca</strong> - see pca, applied for within each cluster<br />")
    txt <- paste(txt, "<strong>clustmcdpca</strong> - see mcdpca, applied for within each cluster<br />")
    txt <- paste(txt, "<strong>clustpppca</strong> - see pppca, applied for within each cluster<br />")
  ...
  output$ui_microagg_aggmeasures <- renderUI({
    ...
    txt_tooltip <- paste(txt, "<strong>mean</strong> - arithmetic mean <br />")
    txt_tooltip <- paste(txt, "<strong>median</strong> - median <br />")
    txt_tooltip <- paste(txt, "<strong>trim</strong> - trimmed mean, specify trimming percentage <br />")
    txt_tooltip <- paste(txt, "<strong>onestep</strong> - for each group, values further than 2.018843 times the median absolute deviation from the group median are replaced by the extreme values of this interval and subsequently the group mean is computed and used as replacement value <br />")
