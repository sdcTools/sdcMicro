...
output$ui_export_problem1 <- renderUI({
  out <- fluidRow(column(12, h4("Save and retrieve current state"), align="center"))
  ...
  out <- list(out, fluidRow(
    column(12, p("The undo button can only be used to go one step back. For experimenting with SDC methods,
      parameters and settings, it can be useful to save a certain state before starting to experiment with different
      SDC methods and, if the result is not satisfactory, revert to the saved state. Here you can save the current
      state and, if necessary, reload this state. Reloading undoes any methods applied to the data since saving the
      last state, but restores any methods applied before the saving. It is also possible to save several states,
      as they are saved on disk."), align="center"),
    ...
    column(12, p("Click here to save the current state with all relevant data and code for reverting to this state later."), align="center"),
...
output$ui_script_import1 <- renderUI({
  ...
  if (!is.null(sdcObj())) {
    out <- list(out, fluidRow(
      column(12, p("Here you can load a previously saved state. The file must be an",code(".rdata"),"file.
      See above for the path where you saved the last state. Please note that uploading a previously saved state
