...
output$ui_recode <- renderUI({
  out <- htmlTemplate("tpl_one_col.html", inp=h4("Recode categorical key variables"))
  out <- list(out,
    htmlTemplate("tpl_one_col.html", inp=p("To reduce risks it is often useful to combine multiple chararacteristics of categorical key variables into
      ...
      has been done, a new label for the new category can be assigned. If you are ready, you just need to press the button.")))
  out <- list(out,
      htmlTemplate("tpl_one_col.html", inp=p("Note: If you only select one level, you still can press the Button and update the key variable. In this case you can rename
        the the selected value.")))
  ...
  if ( length(kv) > 0 ) {
    ...
    out <- list(out,
      ...
      htmlTemplate("tpl_one_col.html", inp=plotOutput("plot_facRec")))
  } else {
    return(noInputData())
  }
  out
...
output$ui_pram <- renderUI({
  ...
  output$ui_expert_pram <- renderUI({
    ...
    if ( length(pramvars) > 0 ) {
      ...
      out <- list(
        htmlTemplate("tpl_one_col.html", inp=h5("Select variables for PRAM")),
        htmlTemplate("tpl_one_col.html", inp=sel_pramvars)
      )
      if (!is.null(input$sel_pramvars_expert)) {
        btn_submit <- myActionButton("btn_pram_expert", label="Postrandomize", btn="primary")
        out <- list(out,
          htmlTemplate("tpl_one_col.html", inp=rHandsontableOutput("transmat", width="100%")),
          htmlTemplate("tpl_one_col.html", inp=btn_submit))
      }
    } else {
      out <- list(
        htmlTemplate("tpl_one_col.html", inp=h5("No Factor-Variables available in the data!"))
      )
    }
    out
  })
  ...
  output$ui_nonexpert_pram <- renderUI({
    ...
    if ( !is.null(input$sel_pramvars_nonexpert) ) {
      ...
      out <- list(out,
        htmlTemplate("tpl_two_col.html", inp1=sl1, inp2=sl2),
        htmlTemplate("tpl_one_col.html", inp=btn_submit))
    }
    out
  })
  ...
  out <- list(
    htmlTemplate("tpl_one_col.html", inp=h4("Postrandomization of categorical variables")),
    htmlTemplate("tpl_one_col.html", inp=p("The algorithm randomly changes the values of selected variables in some records according
      to an invariant probability transition matrix (in non-expert mode) or a custom-defined transition matrix (in expert mode).")),
    htmlTemplate("tpl_one_col.html", inp=p("In non-expert mode, two parameters (",code("pd"),"and",code("alpha"),") must be specified.",
      ...
      expert mode, the user can freely specify a transition matrix which will be used for the post-randomization of a single variable.")),
    htmlTemplate("tpl_one_col.html", inp=p("Please also note that if you have specified a stratification variable when creating the",code("sdcMicroObj"),"postrandomization
      is performed independently on all data-subsets specified by the stratification variable!")))

  btn_expert <- checkboxInput("cbx_expert_pram", label=h4("Use expert settings"),
    value=input$cbx_expert_pram, width="100%")

  out <- list(out, htmlTemplate("tpl_one_col.html", inp=btn_expert))
...
output$ui_kAnon <- renderUI({
  ...
  output$ui_kanon_importanceInputs <- renderUI({
    ...
    for (i in 1:n_rows) {
      ...
      counter <- counter + 1
      out <- list(out, htmlTemplate("tpl_three_col.html", inp1=inp1, inp2=inp2, inp3=inp3))
    }
    out
  })
  ...
  output$ui_kanon_k_combs <- renderUI({
    ...
    for (i in 1:nrKeyVars) {
      out <- list(out, htmlTemplate("tpl_four_col.html",
        inp1=NULL,
        inp2=uiOutput(paste0("rb_kanon_usecombs_",i)),
        inp2=rbs[[i]],
        inp3=sls[[i]], inp4=NULL))
    }
    out
  })

  output$ui_kanon_combs <- renderUI({
    out <- list(htmlTemplate("tpl_three_col.html", inp1=NULL, inp2=uiOutput("ui_kanon_useCombs"), inp3=NULL))
  ...
  out <- list(
    htmlTemplate("tpl_one_col.html", inp=h4("Establish k-anonymity")),
    htmlTemplate("tpl_one_col.html", inp=p("k-Anonymity will be established by suppressing or rather setting to",code(NA),"some values in the categorical key variables.
    ...
    sure that assign low numbers to those variables that you do not want many suppressions in. On the other hand, the higher the importance value for a variable, the larger
      the probability that values will be suppressed within this variable")),
    htmlTemplate("tpl_one_col.html", inp=p("You may also decide to apply the procedure for all possible subsets of key variables. This is useful, if you have many
      key variables. In this case you can choose different values for parameter",code("k"),".")),
    htmlTemplate("tpl_one_col.html", inp=p("Please also note that if you have specified a stratification variable when creating the",code("sdcMicroObj"),"k-anonymity is established
      for all the data-subsets specified by the stratification variable!")))

  #out <- list(out, htmlTemplate("tpl_three_col.html", inp1=NULL, inp2=h5("Importance of keyVars"), inp3=NULL))
  ...
  out <- list(out, htmlTemplate("tpl_three_col.html", inp1=NULL, inp2=rb1, inp3=NULL))
  if (!is.null(input$rb_show_importance) && input$rb_show_importance=="Yes") {
  ...
  out <- list(out, htmlTemplate("tpl_three_col.html", inp1=NULL, inp2=rb2, inp3=NULL))
...
output$ui_supp_threshold <- renderUI({
  ...
  out <- list(
    htmlTemplate("tpl_one_col.html", inp=h4("Supress above given threshold")),
    htmlTemplate("tpl_one_col.html", inp=p("This is a relatively easy method which allows to suppress or rather set to",code("NA"),"values in selected key-variables
      ...
      specified stratification variable")),
    htmlTemplate("tpl_two_col.html", inp1=uiOutput("ui_supp_th"), inp2=uiOutput("ui_supp_th_var")),
    htmlTemplate("tpl_one_col.html", inp=plotOutput("ui_supp_riskplot")))
  btn <- NULL
  if (!is.null(input$sel_supp_th_var)) {
