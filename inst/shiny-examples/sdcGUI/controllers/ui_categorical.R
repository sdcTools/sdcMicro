...
output$ui_pram <- renderUI({
  ...
  out <- htmlTemplate("tpl_one_col.html", inp=btn_expert)
...
output$ui_kAnon <- renderUI({
  ...
  output$ui_kanon_k <- renderUI({
    ...
    sl <- sliderInput("sl_kanon_k", label=h5("k-Anonymity parameter"),
  ...
  rb1 <- radioButtons(inputId="rb_show_importance", label=h5(paste("Modify importance of key-variables for suppression?")),
