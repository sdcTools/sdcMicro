...
{
	  ###########################################################################################################################################
	  test_that("data frame, numeric values (weighted, non-fast method)", {
    ...
    ###########################################################################################################################################
    test_that("data frame, numeric values (weighted, fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1,2,3), w = 4, fast = TRUE)
      expect_equal(unclass(fr$Fk), c(108.0, 45.5, 108.0, 17.0, 100.0, 100.0, 123.5, 108.0))
      expect_equal(unclass(fr$fk), c(3, 1, 3, 1, 2, 2, 1, 3))
      expect_equal(unclass(fr$n1), 3)
      expect_equal(unclass(fr$n2), 2)
    ...
    ###########################################################################################################################################
    test_that("data frame, numeric values (mon-weighted, fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1,2,3), fast = TRUE)
      expect_equal(unclass(fr$Fk), c(3, 1, 3, 1, 2, 2, 1, 3))
      expect_equal(unclass(fr$fk), c(3, 1, 3, 1, 2, 2, 1, 3))
      expect_equal(unclass(fr$n1), 3)
      expect_equal(unclass(fr$n2), 2)
...
{
	###########################################################################################################################################
	test_that("data frame, numeric values with missing values (weighted, non-fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1,2,3), w = 4)
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with missing values (weighted, fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1,2,3), w = 4, fast = TRUE)
      expect_equal(unclass(fr$Fk), c(108.0, 45.5, 125.0, 56.0, 100.0, 100.0, 123.5, 108.0))
      expect_equal(unclass(fr$fk), c(3, 1, 4, 2, 2, 2, 1, 3))
      expect_equal(unclass(fr$n1), 2)
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with missing values (non-weighted, non-fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1,2,3))
      expect_equal(unclass(fr$Fk), c(3, 1, 4, 2, 2, 2, 1, 3))
      expect_equal(unclass(fr$fk), c(3, 1, 4, 2, 2, 2, 1, 3))
      expect_equal(unclass(fr$n1), 2)
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with missing values (non-weighted, fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1,2,3), fast = TRUE)
      expect_equal(unclass(fr$Fk), c(3, 1, 4, 2, 2, 2, 1, 3))
      expect_equal(unclass(fr$fk), c(3, 1, 4, 2, 2, 2, 1, 3))
      expect_equal(unclass(fr$n1), 2)
...
{
	###########################################################################################################################################
	test_that("data frame, numeric values with with one column and one row with NAs (weighted, non-fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1,2,3), w = 4)
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with with one column and one row with NAs (weighted, fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1,2,3), w = 4, fast = TRUE)
      expect_equal(unclass(fr$Fk), c(125.0, 184.5, 394.0, 125.0, 184.5, 184.5, 162.5, 125.0))
      expect_equal(unclass(fr$fk), c(4, 4, 8, 4, 4, 4, 2, 4))
      expect_equal(unclass(fr$n1), 0)
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with with one column and one row with NAs (non-weighted, non-fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1,2,3))
      expect_equal(unclass(fr$Fk), c(4, 4, 8, 4, 4, 4, 2, 4))
      expect_equal(unclass(fr$fk), c(4, 4, 8, 4, 4, 4, 2, 4))
      expect_equal(unclass(fr$n1), 0)
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with with one column and one row with NAs (non-weighted, fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1,2,3), fast = TRUE)
      expect_equal(unclass(fr$Fk), c(4, 4, 8, 4, 4, 4, 2, 4))
      expect_equal(unclass(fr$fk), c(4, 4, 8, 4, 4, 4, 2, 4))
      expect_equal(unclass(fr$n1), 0)
...
{
	###########################################################################################################################################
	test_that("data frame, numeric values with one column with NAs (weighted, non-fast method)", {
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with one column with NAs (weighted, fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1), w = 2, fast = TRUE)
      expect_equal(unclass(fr$Fk), c(133.0, 92.5, 394.0, 133.0, 262.5, 394.0, 262.5, 133.0))
      expect_equal(unclass(fr$fk), c(5, 3, 8, 5, 4, 8, 4, 5))
      expect_equal(unclass(fr$n1), 0)
      expect_equal(unclass(fr$n2), 0)
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with one column with NAs (non-weighted, non-fast method)", {
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with one column with NAs (non-weighted, fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1), fast = TRUE)
      expect_equal(unclass(fr$Fk), c(5, 3, 8, 5, 4, 8, 4, 5))
      expect_equal(unclass(fr$fk), c(5, 3, 8, 5, 4, 8, 4, 5))
      expect_equal(unclass(fr$n1), 0)
      expect_equal(unclass(fr$n2), 0)
...

{
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with one column with NAs (weighted, fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1, 2, 3, 4, 5, 6, 7, 8), w = 9, fast = TRUE)
      expect_equal(unclass(fr$Fk), c(18.0, 167.5, 167.5))
      expect_equal(unclass(fr$fk), c(1, 2, 2))
      expect_equal(unclass(fr$n1), 1)
      expect_equal(unclass(fr$n2), 2)
	...
	###########################################################################################################################################
	test_that("data frame, numeric values with one column with NAs (non-weighted, fast method)", {
      fr <- freqCalc(sample_Data, keyVars = c(1, 2, 3, 4, 5, 6, 7, 8), fast = TRUE)
      expect_equal(unclass(fr$Fk), c(1, 2, 2))
      expect_equal(unclass(fr$fk), c(1, 2, 2))
      expect_equal(unclass(fr$n1), 1)
      expect_equal(unclass(fr$n2), 2)
