...
- Expected reidentifications

We expect 9 re-identifications ( 0.2 %) in the anonymized data set.

In the original dataset we expected 9 ( 0.2 %) re-identifications.

- Expected reidentifications taking cluster-information into account

In this case, we expect to have 44.45 ( 0.97 %) re-identifications given the anonymized data set. For the original data, we expected to have 44.45 ( 0.97 %)) re-identifications.

- Records with large individual reidentification risks

The following (robust) measure calculates the number of records having an individual risk that is larger than the median of all indiviual risks plus two times its Median Absolute Deviation or having an absolute value larger than 0.1 . 0 observations ( 0 in the original data) have a high risk according to this measure.

**Risky Observations** 

Display risky-observations in a Table

`Minimum risk for to be shown in the table` -> slider: 0 and max individual risk

- interactive table with all categorical key variables, fk, Fk and indivRisk


**Plot of risks**

- Histogram showing distribution of individual reidentification-risks
