...
std::vector< std::vector<int> > recordSwap(std::vector< std::vector<int> > data, int hid,
  ...
  // initialize map for household size
  ...
  std::unordered_map<int,int> swappedIndex; // map for indices that have already been used -> unordered map constant time access
  ...
  int z=0; // counter used for while() etc...
  int nhid = 0;
  ...
  for(int h=0;h<nhier;h++){
    ...
    for(auto &x : group_hier_help){
      ...
      if(h<(nhier-1)){
        // in all but the last hierarchy level do the following:
        if(mustSwap.size()>0){
        ...
        // in randSample households that must be swapped are automatically chosen
